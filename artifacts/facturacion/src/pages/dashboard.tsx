        ...
        <Card>
          ...
          <CardContent>
            ...
            {dashboard.overduePayables > 0 && (
              <p className="text-xs text-destructive mt-1 flex items-center gap-1 font-medium">
                <AlertTriangle className="w-3 h-3" />{" "}
                {dashboard.overduePayables} pagos vencidos
              </p>
            )}
          </CardContent>
        </Card>
              ...
              <div className="space-y-4">
                {dashboard.balanceByCompany.map((company) => (
                  <div
                    ...
                    className="flex justify-between items-center p-3 rounded-xl hover:bg-secondary/50 transition-colors"
                  >
        ...
        <Card className={!activeCompanyId ? "" : "lg:col-span-2"}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-amber-500" /> Vencimientos esta
              semana
            </CardTitle>
          </CardHeader>
          <CardContent>
            {dashboard.thisWeekDue.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground text-sm">
                No hay vencimientos pendientes para esta semana.
              </div>
            ) : (
                ...
                {dashboard.thisWeekDue.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <Badge
                        variant={
                          item.type === "receivable" ? "success" : "destructive"
                        }
                ...
                {dashboard.recentInvoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-muted-foreground">
                      {inv.clientName || "Cliente Genérico"}
                    </td>
                    <td className="px-4 py-3">{formatDate(inv.issueDate)}</td>
                    <td className="px-4 py-3">
                      <Badge
                        variant={
                          ["cobrada", "pagada"].includes(inv.status)
                            ? "success"
                            : ["vencida", "anulada"].includes(inv.status)
                              ? "destructive"
                              : "warning"
                        }
